...
import framework.exception.AuthFailedException;
import framework.utility.ClassUtility;
...
// Process Request
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            out.println("Sorry this URL doesn't exist ");
...
// Fields Handlers
    private void handleFields( Object object, Class<?> classs, HttpServletRequest request, List<String> attributes ) throws Exception{
        ...
        for (Field field : fields) {
            ...
            if (this.contains(attributes, name)) {
                Method method = classs.getMethod(ClassUtility.getSetter(field), field.getType());
                Object obj = (field.getType().isArray()) ? request.getParameterValues(name) : request.getParameter(name);
                obj = ClassUtility.cast(obj, field.getType());
...
// Dispatcher
    public void dispatch( HttpServletRequest request, HttpServletResponse response, Object returns ) throws Exception{
        if (returns instanceof ModelView) {
            ...
            if( !view.isJson() && !view.isRedirect() ){
            ...
            }else if( !view.isJson() && view.isRedirect() ){
                this.setDatas(request, data);
                this.setSessions(request, sessions);
                this.handleSession(view, request);
...
// File Handler
    private void handleFile( Class<?> classs, HttpServletRequest request, Object object ){
        ...
        try {
            ...
            for (Field field : fields) {
                if (field.getType() == framework.utility.FileUpload.class) {
                    Method m = classs.getMethod(ClassUtility.getSetter(field), field.getType());
                    Object o = ClassUtility.fileTraitement(files, field);
...
// Additional function
    private Object getInstance( String className ) throws ClassNotFoundException, InstantiationException,NoSuchMethodException, IllegalAccessException, InvocationTargetException{
    ...
    private boolean contains(List<String> datas, String data) {
        for (String str : datas) {
            if (str.trim().equalsIgnoreCase(data)) return true;
        }
        return false;
    }
...
// init method
    ...
    public void init() throws ServletException {
        try {
            ...
            List<Class<?>> cs = ClassUtility.getClasses(packages);
            for (Class<?> c : cs) {
